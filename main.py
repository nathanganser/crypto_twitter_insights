...
from helper import show_tweet, most_important_tweets, get_most_important_low_cap_coins, search_tweets_for_keywords, \
    get_ids_from_usernames, find_new_crypto_projects, find_financing_rounds, show_user, most_retweeted_tweets, search_links_in_tweets
...
def ten_important_coin_telegraph_articles(tweet_set):
    ...
    tweets_with_article = search_links_in_tweets(tweet_set, "cointelegraph.com")
    if tweets_with_article:
        for t in tweets_with_article:
            show_tweet(t)
            print('---------')
    else:
